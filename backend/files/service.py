...
class FileSizeLimitExceededError(ValueError): pass
class InvalidFileTypeError(ValueError): pass
class FileNotFoundError(ValueError): pass

def get_user_files_cache_key(user_id: int) -> str:
...
async def validate_and_get_file_size(file_obj) -> int:
    ...
    if size > MAX_FILE_SIZE:
        raise FileSizeLimitExceededError("File size exceeds 10MB limit")
    return size

def validate_mime_type(mime_type: str):
    if mime_type not in ALLOWED_MIME_TYPES:
        raise InvalidFileTypeError("Invalid file type. Allowed: .png, .jpg, .pdf, .txt")
...
async def upload_file_service(file_obj, current_user: User, db: AsyncSession) -> File:
    ...
    await redis_client.delete(get_user_files_cache_key(current_user.id))
    return file_record

async def list_user_files(current_user: User, db: AsyncSession) -> list[File]:
    cache_key = get_user_files_cache_key(current_user.id)
...
async def download_file_service(file_id: int, current_user: User, db: AsyncSession):
    ...
    async def stream_generator():
        async with session.client(
            ...
            s3_obj = await stream_client.get_object(Bucket=settings.BUCKET_NAME, Key=file_record.storage_key)
...
async def delete_file_service(file_id: int, current_user: User, db: AsyncSession, s3_client):
    ...
    await s3_client.delete_object(Bucket=settings.BUCKET_NAME, Key=file_record.storage_key)
...
async def generate_share_link_service(file_id: int, expires_in: int, current_user: User, db: AsyncSession) -> str:
    file_record = await get_file_by_id(file_id, current_user, db)
    ...
    async with session.client(
        ...
        url = await s3.generate_presigned_url(
            'get_object',
            Params={'Bucket': settings.BUCKET_NAME, 'Key': file_record.storage_key},
